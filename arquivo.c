...
long salvar_veiculo_arquivo(Veiculo v) {
  ...
  fprintf(arq, "%0*d|%-20s|%s|%s|%s|%s|%s|%d|%.2f|%d|%d\n", TAM_ID, v.id,
    ...
    if (fgets(linha, sizeof(linha), arq) != NULL) {
        ...
        if (token) {
            ...
            for(int i = strlen(v.marca) - 1; i >= 0; i--) {
                if(v.marca[i] == ' ') v.marca[i] = '\0';
                else break;
            }
        }

        token = strsep_custom(&linha_ptr, "|");
        if (token) strncpy(v.modelo, token, MAX_MODELO - 1);

        token = strsep_custom(&linha_ptr, "|");
        if (token) strncpy(v.ano, token, MAX_ANO - 1);

        token = strsep_custom(&linha_ptr, "|");
        if (token) strncpy(v.cor, token, MAX_COR - 1);

        token = strsep_custom(&linha_ptr, "|");
        if (token) strncpy(v.combustivel, token, MAX_COMBUSTIVEL - 1);

        token = strsep_custom(&linha_ptr, "|");
        if (token) strncpy(v.cambio, token, MAX_CAMBIO - 1);

        token = strsep_custom(&linha_ptr, "|");
        if (token) v.portas = atoi(token);

        token = strsep_custom(&linha_ptr, "|");
        if (token) v.preco = atof(token);

        token = strsep_custom(&linha_ptr, "|");
        if (token) v.km = atoi(token);

        token = strsep_custom(&linha_ptr, "|");
        if (token) v.status = atoi(token);
    }
